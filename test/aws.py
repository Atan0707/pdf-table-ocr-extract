...
try:
    ...
    if total_pages < start_page:
        log_print(f"Error: PDF only has {total_pages} page(s). Page {start_page} does not exist.")
        log_file.close()
        exit(1)
    
    # Adjust end_page if PDF has fewer pages
    if total_pages < end_page:
        log_print(f"Warning: PDF only has {total_pages} pages. Processing up to page {total_pages}.")
        end_page = total_pages
    
    log_print(f"Processing pages {start_page} to {end_page}...")
    log_print(f"Total pages in PDF: {total_pages}")
    log_print("="*60)
